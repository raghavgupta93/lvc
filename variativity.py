...
def variativity_replacement(sentence, verb_token, object_token, object_index, verb_index, parsed_tokens, related_verb, lvc_phrase, catvar_file, adjective_adverb_dict, no_adverb_set, object_preposition_dict, no_preposition_objects, subject_person, subject_number):
	...
	if len(phrases) != 2:
		return ''"""
	final_phrase_active = nodebox_verb_conjugator(related_verb, list_of_auxiliary_verbs, subject_person, subject_number, nodebox_negate, nodebox_participle, nodebox_gerund, nodebox_simple_present_third_person, nodebox_simple_present_other_person, nodebox_simple_past) #phrases[0]
	final_phrase_passive = nodebox_verb_conjugator_passive(related_verb, final_phrase_active, list_of_auxiliary_verbs, subject_person, subject_number, nodebox_negate, nodebox_participle, nodebox_gerund, nodebox_simple_present_third_person, nodebox_simple_present_other_person, nodebox_simple_past) #phrases[1]
		...
		if '_FROM' in verb_prep_caps:
			...
			if verb_prep_combination_dict[verb_prep_caps] == '+':
				...
				if verb_probability - verb_prep_probability > 1.75:	
					...
					probabilities = urllib2.urlopen(urllib2.Request('http://weblm.research.microsoft.com/rest.svc/bing-body/2013-12/5/jp?u=' + microsoft_weblm_api_key + '&format=json', 'been ' + en.verb.present_participle(related_verb) + '\n' + 'been ' + en.verb.present_participle(related_verb) + ' ' + preposition_token.orth_)).read().split(',')
					verb_probability = float(probabilities[0][1:])
					verb_prep_probability = float(probabilities[1][:-1])
					if verb_probability - verb_prep_probability > 1.75:	
